-- Save a task's release and project schedule in one statement.
--
-- The project-chain trigger validates both columns together. Updating release_id
-- first and portfolio_plan_id afterwards creates a temporary mismatched state
-- while editing an existing task, even when the two newly selected records belong
-- to the same QA project.
...
declare
  ...
  resolved_portfolio_plan_id uuid;
...
begin
  ...
  if requested_release_id is null then
    raise exception 'Release is required';
  end if;

  select project_id
  into release_project_id
  ...
  allocation_start_month := date_trunc(
    'month',
    coalesce(nullif(task_payload ->> 'allocation_start_date', '')::date, current_date)
  )::date;
  allocation_end_month := date_trunc(
    'month',
    coalesce(nullif(task_payload ->> 'allocation_end_date', '')::date, allocation_start_month)
  )::date;

  -- Accept the client suggestion only when it belongs to the release project
  -- and covers the complete task date range. Otherwise resolve it from the
  -- canonical project and dates.
  select plan.id
  into resolved_portfolio_plan_id
  from public.project_monthly_plans plan
  where plan.id = nullif(task_payload ->> 'portfolio_plan_id', '')::uuid
    and plan.project_id = release_project_id
    and plan.status <> 'cancelled'
    and plan.plan_month <= allocation_start_month
    and coalesce(plan.end_month, plan.plan_month) >= allocation_end_month;

  if resolved_portfolio_plan_id is null then
    select plan.id
    into resolved_portfolio_plan_id
    from public.project_monthly_plans plan
    where plan.project_id = release_project_id
      and plan.status <> 'cancelled'
      and plan.plan_month <= allocation_start_month
      and coalesce(plan.end_month, plan.plan_month) >= allocation_end_month
    order by
      case plan.status when 'confirmed' then 0 when 'planned' then 1 else 2 end,
      plan.plan_month desc,
      plan.created_at desc
    limit 1;
  end if;

  if resolved_portfolio_plan_id is null then
    raise exception 'No project schedule covers the task date range';
  end if;

  target_task_id := public.save_qa_task_with_assignees(task_payload, assignee_payload);

  -- Keep these two project-bound foreign keys in the same UPDATE so the
  -- validation trigger never sees the old schedule with the new release (or
  -- the old release with the new schedule).
...
comment on function public.save_qa_task_workflow(jsonb, jsonb) is
  'Secure task workflow save with atomic release and project-schedule linkage';
