...
| 人员档案 | `src/modules/departments/` | 已模块化 | 第一阶段已覆盖部门、员工档案和供应商基础 |
| 项目总览 | `src/modules/projects/` | 已模块化 | 维护即可 |
| 工作事项 | `src/modules/tasks/` + `main.html` 兼容层 | 部分模块化 | 分阶段迁移编辑器、进度和 TestHub 计算 |
| 版本与发布 | `src/modules/releases/` + `main.html` 兼容层 | 已模块化 | 新功能只维护模块，后续删除旧兼容实现 |
| 质量报表 | `src/modules/reports/` + `main.html` 展示兼容层 | 部分模块化 | 查询、筛选、指标、任务/BUG 明细视图模型、成员分摊、复盘统计、对外简报统计和周简报分组已迁移 |
| 项目总排期 | `main.html` | 未迁移 | 数据依赖多，排在版本之后 |
| BUG 管理 | `main.html` | 未迁移 | 可独立迁移 |
| 账号与权限 | `main.html` | 未迁移 | 涉及权限与密码重置，需单独 PR |
...
1. **质量报表第九阶段**：迁出复制、CSV 与复盘文案生成，保持统计口径不变；
2. **项目总排期**：复用项目、工作事项和服务器完成时间；
3. **BUG 管理**：整理 BUG、复盘和负责人关联；
4. **全功能测试地图**；
5. **账号与权限**：同时复核 RLS、职责和管理员 Edge Function；
6. **工作事项第二阶段**：迁出编辑器、手工进度和 TestHub 计算；
7. **工作台**：最后迁移跨模块聚合与跳转；
8. 统一 UI、性能、自动化回归和无障碍体验。
